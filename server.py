...
MAX_BODY = 10 * 1024 * 1024  # 10MB hard limit on request body reads
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/exit"):
            ...
            if token != os.environ.get("IFLYVOICE_ADMIN_TOKEN", "local-only"):
    ...
    def _proxy(self, method):
        ...
        if method == "POST":
            ...
            if cl > 0:
                try:
                    body = self.rfile.read(min(cl, MAX_BODY))
                except Exception as e:
    ...
    def _load_api_key(env_name, secret_file="~/.iflyvoice/secrets"):
        """Load API key from env var, then config file, then empty string."""
    ...
    SENSEVOICE_CONFIG = {
        ...
        "api_key": "",  # set at class init below
        "model": "SenseVoiceSmall",
    }
    SENSEVOICE_CONFIG["api_key"] = _load_api_key("XINFERENCE_API_KEY")
    ...
    def _handle_tts(self):
        ...
        try:
            original_body = self.rfile.read(min(cl, MAX_BODY))
    ...
    def _handle_chat_completions(self):
        ...
        try:
            body = json.loads(self.rfile.read(min(cl, MAX_BODY)).decode("utf-8"))
        except Exception as e:
        ...
        if not audio_data:
            ...
            try:
                ...
                ds_api_key = Handler._load_api_key("DEEPSEEK_API_KEY")
